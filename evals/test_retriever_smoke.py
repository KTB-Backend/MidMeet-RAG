...
from rag_core.retriever import RETRIEVAL_TOP_K_ENV, search_atmosphere
...
class FakeEmbeddingAdapter(EmbeddingAdapter):
    model_name = "fake-retriever-smoke"
...
def test_retriever_returns_quiet_seed_as_top_match(tmp_path) -> None:
    chroma_path = tmp_path / "chroma"
    collection_name = "venues_retriever_smoke"
    ...
    results = search_atmosphere(
        "조용히 대화하기 좋은 카페",
        embedder,
        chroma_path=chroma_path,
        collection_name=collection_name,
        environ={RETRIEVAL_TOP_K_ENV: "2"},
    )

    assert len(results) == 2
    assert results[0].place_id == "retriever_001"
    assert results[0].name == "스모크 조용한 카페"
    assert results[0].atmosphere_score > results[1].atmosphere_score
    assert results[0].distance < results[1].distance
    assert results[0].attributes.startswith("{")
    ...
    *,
    ...
    return SeedRecord(
        ...
        source=SeedSource(
            ...
            reference="retriever smoke test",
